...
class Graph(object):
    ...
    def add(self, node):
        ...
        self.gradients[node] = np.zeros_like(self.output[node])
    ...
    def backprop(self):
        ...
        "*** YOUR CODE HERE ***"
        index = len(self.get_nodes()) - 1

        while index >= 0:
            current_node = self.get_nodes()[index]
            if current_node == loss_node:
                np.place(self.gradients[loss_node], self.gradients[loss_node] == 0, 1)
            output = current_node.backward(self.get_inputs(current_node), self.gradients[current_node])
            i = 0
            while i < len(current_node.get_parents()):
                # self.gradients[current_node.get_parents()[i]] += output[i]
                self.gradients[current_node.get_parents()[i]] = output[i] + self.gradients[current_node.get_parents()[i]]
                i += 1
            index -= 1
...
class MatrixMultiply(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        dA = np.dot(gradient, inputs[1].T)
        dB = np.dot(inputs[0].T, gradient)
        return [dA, dB]
...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        return [gradient, np.dot(ones, gradient)]
...
class ReLU(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        # inputs[0][np.where(inputs[0] < 0)] = 0
        # return inputs[0]
        return np.where(inputs[0] < 0, 0, inputs[0])

    @staticmethod
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        partial = gradient.copy()
        partial[np.where(inputs[0] < 0)] = 0
        return [partial]
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        dA = inputs[0] - inputs[1]
        dB = -dA
        return [np.dot(gradient, dA)/inputs[0].size, np.dot(gradient, dB)/inputs[0].size]
