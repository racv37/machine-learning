...
class RegressionModel(Model):
    ...
    def __init__(self):
        ...
        self.m1 = nn.Variable(1, 400)
        self.b1 = nn.Variable(200, 400)
        self.m2 = nn.Variable(400, 200)
        self.b2 = nn.Variable(200, 200)
        self.m3 = nn.Variable(200, 1)
        self.b3 = nn.Variable(200, 1)
    ...
    def run(self, x, y=None):
        ...
        """
        "*** YOUR CODE HERE ***"
        graph = nn.Graph([self.m1, self.b1])
        graph.add(self.m2)
        graph.add(self.b2)
        graph.add(self.m3)
        graph.add(self.b3)
        ...
        relum2 = nn.MatrixMultiply(graph, relu, self.m2)
        relum2_plus_b2 = nn.MatrixVectorAdd(graph, relum2, self.b2)
        relu2 = nn.ReLU(graph, relum2_plus_b2)

        relum3 = nn.MatrixMultiply(graph, relu2, self.m3)
        relum3_plus_b3 = nn.MatrixVectorAdd(graph, relum3, self.b3)
        ...
        if y is not None:
            ...
            loss = nn.SquareLoss(graph, relum3_plus_b3, input_y)
            return graph
        else:
            # At test time, the correct output is unknown.
            # You should instead return your model's prediction as a numpy array
            "*** YOUR CODE HERE ***"
            return graph.get_output(relum3_plus_b3)
...
class OddRegressionModel(Model):
    ...
    def __init__(self):
        ...
        self.learning_rate = 0.01
        self.m1 = nn.Variable(1, 400)
        self.b1 = nn.Variable(16, 400)
        self.m2 = nn.Variable(400, 200)
        self.b2 = nn.Variable(16, 200)
        self.m3 = nn.Variable(200, 1)
        self.b3 = nn.Variable(16, 1)
    ...
    def run(self, x, y=None):
        ...
        graph.add(self.m3)
        graph.add(self.b3)
        neg_one = np.ones(1).reshape((1, 1)) * -1
        neg = nn.Input(graph, neg_one)
        ...
        neg_xm = nn.MatrixMultiply(graph, neg_input_x, self.m1)
        neg_xm_plus_b = nn.MatrixVectorAdd(graph, neg_xm, self.b1)
        neg_relu = nn.ReLU(graph, neg_xm_plus_b)

        neg_relum2 = nn.MatrixMultiply(graph, neg_relu, self.m2)
        neg_relum2_plus_b2 = nn.MatrixVectorAdd(graph, neg_relum2, self.b2)
        neg_relu2 = nn.ReLU(graph, neg_relum2_plus_b2)

        neg_relum3 = nn.MatrixMultiply(graph, neg_relu2, self.m3)
        neg_relum3_plus_b3 = nn.MatrixVectorAdd(graph, neg_relum3, self.b3)
        neg_func = nn.MatrixMultiply(graph, neg_relum3_plus_b3, neg)

        g = nn.Add(graph, relum3_plus_b3, neg_func)
        ...
        if y is not None:
            ...
            "*** YOUR CODE HERE ***"
            input_y = nn.Input(graph, y)
            loss = nn.SquareLoss(graph, g, input_y)
            return graph
        else:
            # At test time, the correct output is unknown.
            # You should instead return your model's prediction as a numpy array
            "*** YOUR CODE HERE ***"
            return graph.get_output(g)
...
class DigitClassificationModel(Model):
    ...
    def __init__(self):
        ...
        self.b1 = nn.Variable(100, 400)
        self.m2 = nn.Variable(400, 200)
        self.b2 = nn.Variable(100, 200)
        self.m3 = nn.Variable(200, 10)
        self.b3 = nn.Variable(100, 10)
